(function(Prismic) {
  ...
  asyncTest('Handles multiple fields', 1, function() {
    Prismic.Api(testRepository, function(err, Api) {
      ...
      Api.form('everything').query('[[:d = at(document.id, "UlfoxUnM0wkXYXbX")]]').ref(Api.master()).submit(function(err, documents) {
        if (err) { console.log(err); return; }
        equal(documents.results[0].getAll('blog-post.relatedpost')[0].asHtml(ctx), '<a href="/testing_url/UlfoxUnM0wkXYXbm/tips-to-dress-a-pastry?ref=XXXXX">/testing_url/UlfoxUnM0wkXYXbm/tips-to-dress-a-pastry?ref=XXXXX</a>');
        start();
      });
    }, previewToken);
  });
