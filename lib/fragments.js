    ...
    if (Array.isArray(this.blocks)) {
      ...
      for(var i=0; i < this.blocks.length; i++) {
        ...
        if (block.type == "image" && block.linkTo) {
          ...
          if (link.value && link.value.target) {
            block.linkTarget = link.value.target;
          }
        }
...
function serialize(element, content, htmlSerializer) {
  ...
  if (element.type == "image") {
    ...
    var imageTarget = element.linkTarget ? ' target="' + element.linkTarget + '"' : '';
  ...
  if (element.type === 'hyperlink') {
    var target = (element.data.value && element.data.value.target) ? ' target="'+ element.data.value.target +'"' : '';
